...
void* ffi_usleep(int nano_seconds);
void* ffi_msleep(int microseconds);
void* ffi_sleep(int seconds);
...
{
#ifdef _WIN32
	ffi_raw("kernel32","Sleep",0)(seconds*1000);
#else
	ffi_raw("libc","usleep",0)(seconds*1000000);
#endif
	return 0;
}
void* ffi_msleep(int microseconds)
{
#ifdef _WIN32
	ffi_raw("kernel32","Sleep",0)(microseconds);
#else
	ffi_raw("libc","usleep",0)(microseconds*1000);
#endif
	return 0;
};
void* ffi_usleep(int nano_seconds)
{
#ifdef _WIN32
	ffi_raw("kernel32","Sleep",0)(nano_seconds/1000);
#else
	ffi_raw("libc","usleep",0)(nano_seconds);
#endif
	return 0;
};
