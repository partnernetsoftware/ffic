...
#define ffic_import_(c,n) ffic_func c##_##n = (ffic_func) ffic_raw(#c,#n,0)
...
ffic_ptr(*ffic(const char* libname, const char* funcname))()
{
	...
	if(!libname){//special libc tuning
		if(!_ffic_os_sleep) _ffic_os_sleep = (ffic_os==ffic_os_win) ? (ffic_func)ffic_raw("kernel32","Sleep",0) : (ffic_func)ffic_raw(ffic_libcname,"usleep",0);
